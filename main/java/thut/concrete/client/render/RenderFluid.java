...
public class RenderFluid implements ISimpleBlockRenderingHandler 
{
    ...
    public float getOverLayHeightForRender(IBlockAccess world, int x, int y, int z, Block block) {
        ...
        if (world.getBlock(x, y, z) == block) {
            if (meta>7||world.getBlock(x, y + 1, z) == block) {
                return 1;
            }
            if (meta == 0) {
                return 0.875F;
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        ...
        if(block instanceof IRebar)
        {
    		...
    		{
            	BlockLiquidREConcrete blo = (BlockLiquidREConcrete)block;
            	icon = blo.theIcon;
            	rebar = meta!=15;
            }
    		else if(block instanceof BlockREConcrete)
            {
            	BlockREConcrete blo = (BlockREConcrete)block;
            	icon = blo.theIcon;
            	rebar = meta!=15;
            }
    		if(rebar)
        ...
        boolean renderTop = world.getBlock(x, y + 1, z) != theFluid;

        boolean renderBottom = block.shouldSideBeRendered(world, x, y - 1, z, 0) && world.getBlock(x, y - 1, z) != theFluid;
        ...
        if (!renderTop && !renderBottom && !renderSides[0] && !renderSides[1] && !renderSides[2] && !renderSides[3]) {
            ...
            if (renderer.renderAllFaces || renderTop) {
                ...
                IIcon iconStill = block.getIcon(world, x, y, z, 1);
...
//                heightNE -= RENDER_OFFSET;
            ...
            if (renderer.renderAllFaces || renderBottom) {
                ...
                renderer.renderFaceYNeg(block, x, y, z, block.getIcon(world, x, y, z, 0));//y + RENDER_OFFSET
            ...
            for (int side = 0; side < 4; ++side) {
                ...
                IIcon iconFlow = block.getIcon(world, x, y, z, side+2);
    ...
    public boolean renderFluidOverLay(IBlockAccess world, int x, int y, int z, Block block, RenderBlocks renderer)
    {
        ...
        boolean[] renderSides = new boolean[] { world.getBlock( x, y, z - 1)!=block, world.getBlock( x, y, z + 1)!=block,
        		world.getBlock( x - 1, y, z)!=block,world.getBlock( x + 1, y, z)!=block };
        
        
        if((up==Blocks.water||up==Blocks.flowing_water))
        {
            tessellator.setBrightness(block.getMixedBrightnessForBlock(world, x, y, z));
            tessellator.setColorOpaque_F(LIGHT_Y_POS * red, LIGHT_Y_POS * green, LIGHT_Y_POS * blue);
...
//                }
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID,
		...
		new RenderCuboid(t, new IIcon[] {block.getIcon(0, metadata)}, 0, 0, 0, 1, 1, 1);
		t.draw();
